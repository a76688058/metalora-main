...
function MacroEdgeModel({ scrollProgress }: { scrollProgress: any }) {
  ...
  const materials = useMemo(() => {
    ...
    const surfaceMaterial = new THREE.MeshStandardMaterial({
      color: theme === 'dark' ? '#050505' : '#f5f5f5',
      metalness: 0.9,
      roughness: 0.15,
    });
  ...
  return (
    <>
      ...
      <ambientLight intensity={theme === 'dark' ? 0.05 : 0.5} />
      ...
      <pointLight
        ref={backLightRef}
        position={[-3, 0, -2]}
        intensity={theme === 'dark' ? 150 : 300}
        color={theme === 'dark' ? "#4a4a4a" : "#e5e5e5"}
      />

      <pointLight position={[-2, 0, 2]} intensity={theme === 'dark' ? 40 : 80} color={theme === 'dark' ? "#ffffff" : "#000000"} />
...
export default function MaterialEdgeAnimation() {
  ...
  return (
    <section ref={containerRef} className={`relative h-[300vh] transition-colors duration-500 ${theme === 'dark' ? 'bg-[#000000]' : 'bg-[#ffffff]'}`}>
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        >
          <div className={`absolute inset-0 ${
            theme === 'dark' ? 'bg-[radial-gradient(circle_at_60%_50%,#1a1a1a_0%,transparent_70%)]' : 'bg-[radial-gradient(circle_at_60%_50%,#f5f5f5_0%,transparent_70%)]'
          }`} />
          <div className={`absolute inset-0 opacity-50 ${
            theme === 'dark' ? 'bg-[radial-gradient(circle_at_40%_50%,#0f172a_0%,transparent_60%)]' : 'bg-[radial-gradient(circle_at_40%_50%,#e5e7eb_0%,transparent_60%)]'
          }`} />
        </motion.div>
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          <motion.div style={{ opacity: modelOpacity }} className="absolute inset-0 z-0">
            <ErrorBoundary fallback={
              <div className={`w-full h-full ${theme === 'dark' ? 'bg-gradient-to-r from-zinc-900 to-black' : 'bg-gradient-to-r from-zinc-100 to-white'}`} />
            }>
              <Canvas 
                frameloop="always" 
                dpr={[1, 1.5]}
          ...
          <div className={`absolute inset-y-0 left-0 w-1/2 z-5 pointer-events-none ${
            theme === 'dark' ? 'bg-gradient-to-r from-black/40 to-transparent' : 'bg-gradient-to-r from-white/40 to-transparent'
          }`} />
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto px-6 pointer-events-none">
            ...
            >
              <div className="space-y-4">
                <h2 className={`text-5xl md:text-8xl font-thin tracking-tight leading-tight ${theme === 'dark' ? 'text-[#FFFFFF]' : 'text-[#000000]'}`}>
                  <span className={`font-bold bg-clip-text text-transparent ${
                    theme === 'dark' ? 'bg-gradient-to-r from-zinc-300 via-white to-zinc-300' : 'bg-gradient-to-r from-zinc-700 via-black to-zinc-700'
                  }`}>
                    1.15mm
                  </span>
                  의 영속성.
                </h2>
                <p className={`text-xl md:text-2xl font-thin tracking-[0.1em] max-w-xl leading-relaxed ${
